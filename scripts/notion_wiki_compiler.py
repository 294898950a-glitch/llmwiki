...
def judge_concept_candidates(
    ...
    system_rules = (
        ...
        "- 候选要和当前主题**在同一领域**——领域不同就绝对不能关联：\n"
        "  · agent 架构 / runtime / state / recovery / context → agent 域\n"
        "  · 量化 / 因子 / 回测 / 可转债 / 强赎 → 金融 / 量化域\n"
        "  · 优化算法 / CMA-ES / 贝叶斯优化 / 黑箱优化 → 优化 / 数值算法域\n"
        "  · 供应链 / 产能 / 商业 → 商业域\n"
        "  跨域不可关联（例：CMA-ES 和 State Management 仅字面都有'state'但**领域完全不同**，NOT a match）\n"
        "- 避免超级泛（如'概念'、'系统'）\n"
        f"- 最多返回 {max_candidates} 个；**宁空勿错**——如果 pool 里没真正同域邻居，返回空 `{{\"candidates\": [], \"reasoning\": \"pool 无同域邻居\"}}` 比硬选强\n"
        f"\n{pool_hint}\n"
        "\n"
        "输出格式（严格）：\n"
