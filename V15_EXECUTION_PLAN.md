...
代码：
- [x] 已做：`log-session-event` ✅、`--emit-diff` ✅
- [x] `list-review-queue` 子命令（v18 P0 闭环的第一步，2026-04-23 落地）：
  - 四路信号聚合：`check-editorial` yellow/red + audit-log `review_required: true` + `Verification = Needs Review` + compile-queue failures
  - `--emit-decisions` 把新信号写入 `raw/notion_dumps/decisions.jsonl`
  - sha1 截断 id 幂等去重；已 resolved|dropped 的 id 不再次 raise
- [x] `resolve-decision <id> --status in_review|resolved|dropped --rationale "..."` 子命令：decision 生命周期的 append-only 状态推进

文档：
- [ ] 新文档 `REVIEW_QUEUE.md`：定义 review queue 的输入源、优先级排序、会话层处理流程（decisions.jsonl schema 当前写在 CLAUDE.md 脚本段；正式化时迁出）
...
- [ ] `list-review-queue` 输出的 yellow/red 页面数量随时间下降（子命令已实现，需累计运行数据后判断）
...
- ~`list-review-queue` 子命令未实现（§2 阶段 3）~ → 已实现（2026-04-23）；还需累计运行几轮数据后判断是否有效降低悬案

**下阶段建议**（不再由 v15 驱动，改由 v16 后的真实需求驱动）：

1. 把 QueryLoop 推到 `check-editorial` / `reference-check` 双 green，作为真正样板页
2. ~实现 `list-review-queue`~ → 已落地；下一层是 v18 P1（对象生命周期 growing/stable/stale/conflicted 状态机）+ P2（check-editorial / llm-validate / reference-check 汇成单一 page quality state）
